...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      // 1. Carica il patient salvato nelle SharedPreferences.
      // Questo serve per evitare "No user data found".
      await patientState.loadFromPreferences();

      // 2. Manteniamo anche la vecchia logica della dashboard.
      // Se questi dati esistono ancora nelle SharedPreferences, li usiamo.
      ...
      if (name != null &&
        ...
        patientState.updateWeight(loadedUser.weight);
      }

      // 3. Nuova parte: prende i dati IMPACT da inizio giornata
      // fino all'ora attuale del dispositivo.
    ...
    return SafeArea(
      child: Consumer<FoodDiaryDB>(
        builder: (context, foodDiaryDB, child) {
          ...
          final exerciseCalories = patientState.burnedCalories;
          ...
          for (final entry in foodDiaryDB.entries) {
            ...
            if (catalogItem != null) {
              consumedMap[catalogItem] =
                  (consumedMap[catalogItem] ?? 0) + entry.grams;
            }
          }
          ...
          final allScores = NutritionEngine.rankFoods(
            ...
            stepsToday: patientState.steps,
          );

          final topSuggestions = allScores
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  _scoreColor(score.totalScore, colorScheme).withOpacity(0.4),
            ),
          ),
          child: Text(
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () =>
                        Navigator.pushNamed(context, '/consumed-calories'),
                    child: Container(
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () =>
                        Navigator.pushNamed(context, '/burned-calories'),
                    child: Container(
                      padding: const EdgeInsets.symmetric(
                        horizontal: 14,
                        vertical: 12,
